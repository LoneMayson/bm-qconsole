...
Процедура КомандаОткрытьЭлемент(Команда)
	...
	Если Команда.Имя = "КомандаОткрытьФормуОбработки" Тогда
		ОткрытьФормуОбъекта(ПараметрыОткрытия);
	Иначе
		Если ТипЗнч(ПараметрыОткрытия.Ссылка) = Тип("Структура") Тогда
		...
		Иначе
			ПоказатьЗначение(, ПараметрыОткрытия.Ссылка);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ДекорацияИсключениеНажатие(Элемент)
	
	ПоказатьЗначение(, СтрСоединить(ЭтотОбъект.КолонкиИсключений.ВыгрузитьЗначения(), ", "));
...
Процедура СоздатьДинамическийСписок(Знач Описание, Знач Настройки)
	...
	Элементы.ДекорацияИсключение.Видимость = (ЭтотОбъект.КолонкиИсключений.Количество() > 0);
